...
public class EmployeeServiceImpl implements EmployeeService, UserDetailsService {
    ...
    public Employee create(String name, String email, String password, EmployeeType type, List<Long> skillId, LocalDate employmentDate) {
        ...
        for (Long id : skillId) {
            skills.add(skillRepository.findById(id).orElseThrow(InvalidEmployeeIdException::new));
        }
    ...
    public List<Employee> filter(Long skillId, Integer yearsOfService) {
        ...
        if (skillId == null && yearsOfService == null) {
            ...
            result = employeeRepository.findAllBySkillsContainingAndEmploymentDateBefore(skillRepository.getById(skillId),
                    LocalDate.now().minusYears(yearsOfService));
        } else if (skillId == null) {
            result = employeeRepository.findAllByEmploymentDateBefore(LocalDate.now().minusYears(yearsOfService));
        } else {
            result = employeeRepository.findAllBySkillsContaining(skillRepository.getById(skillId));
        }
        return result;
    }
